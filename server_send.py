...
import socket, sys, threading,os,subprocess
filesize = 4096
DiN = {}
Queue = []
cmd_queue = []
        ...
        try :
         ...
         if not msgClient or msgClient.upper() =="FIN":
          break
           ...
           id_format = UDecode.split("µ")
           IdClient = id_format[0].split(":")[1]
           DiN[nom] = IdClient
           if id_format[1].split(":")[1] != password:
              break  
         elif IdMode == "$" :
           for j in admin :
              if j == IdClient :
